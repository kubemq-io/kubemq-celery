"""App Tasks — KubeMQ Celery Django Integration.

Demonstrates:
- Email sending with retry logic and exponential backoff
- Report generation with progress tracking via update_state
- File upload processing with idempotency
- Periodic tasks: data cleanup, health checks, cache warming, digest emails

Usage:
    Import tasks and call .delay() or .apply_async() from views or management commands.
    Periodic tasks are scheduled via CELERY_BEAT_SCHEDULE in settings.py.
...
@shared_task
def warm_cache() -> dict:
    """Pre-populate cache with frequently accessed data (periodic task).

    Runs every 30 minutes to keep hot data in cache, reducing
...
def send_daily_digest() -> dict:
    """Compile and send daily activity digest emails (periodic task).

    Runs once per day at 8:00 AM UTC. Gathers activity from the
    past 24 hours and sends summary emails to subscribed users.
    """
    logger.info("Compiling daily digest")
    time.sleep(1.0)
    return {
        "users_notified": 128,
        "activities_summarized": 456,
        "status": "sent",
    }
